...
unsigned long intervalo = 60000UL * 15; // 15 minutos
...
void loop() {
  ...
  estado_atual = atualizarEstado(umidade_atual);
  enviarEstadoParaApp(estado_atual);
  verificarComandos(); // checa se o user apertou o BOTAO_REGA e liga a bomba dagua
}

void atualizarEstado(umidade) {
  int estado;
  if (umidade < 20) {
    estado = 1;          // SECO
  } else if (umidade < 60) {
...
void enviarEstadoParaApp(estado) {
...
int lerUmidade() {
  ...
  int umidade = map(umidade_cru, 0, 1023, 0, 100);
  return umidade;
}
